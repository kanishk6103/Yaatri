...
export default function FindTrainsScreen({ navigation }) {
    ...
    const searchTrains = async () => {
        if ((fromValue.length === 0) || (toValue.length === 0) || (date.length === 0)) return;
        ...
        <View className="h-screen bg-black px-6">
            <View className="">
                <View className="flex flex-col space-y-2 justify-evenly my-2">
                    ...
                    <View>
                        <InputFindTrains label={"To"} placeholder={"Enter destination station.."} type={"Text"} value={toValue} onChangeText={(text) => setToValue(text.toUpperCase())} />
                    </View>
                </View>
                <View>
