            ...
            <View className="w-max mx-6 bg-Dark px-3 py-2 rounded-2xl">
                ...
                <View className="flex flex-row justify-between items-baseline">
                    ...
                    <View className="w-1/3 relative">
                        ...
                        <Text className="text-SecondaryFont text-[10px] absolute -top-2 left-[25%]">{formattedDuration}</Text>
                    </View>
                    {/* Destination */}
                    <View className="flex flex-col gap-1 mt-2 justify-end items-end w-1/3">
                        {/* Time */}
                        <View><Text className="text-OffWhite text-xs">{to_std}</Text></View>
                        {/* Destination Station */}
                        <View><Text className="text-SecondaryFont text-xs">{to_station_name.length > 16 ? to_station_name.slice(0, to_station_name.length / 1.8) + '...' : to_station_name}</Text></View>
                        {/* Date */}
                        <View><Text className="text-SecondaryFont text-xs">{arrivalDate}</Text></View>
                    </View>
                </View>
                <View className="flex flex-col gap-1 my-1 space-y-2">
                    <View className="flex flex-col">
                        {/* Available Classes */}
                        <Text className="text-xs font-bold text-[#666666]">Available Classes:</Text>
                        <View className="flex flex-row gap-2 w-max flex-wrap">
                            {
                                class_type.map((singleClass, index) => {
                                    return (
                                        <Text className={'px-3 py-2 rounded text-sm font-bold text-OffWhite bg-InputBG'} key={index}>
                                            {singleClass}
                                        </Text>
                                    )
                                })
                    ...
                    <View className="flex flex-col">
                        {/* Running Days */}
                        <Text className="text-xs font-bold text-[#666666]">Operating Days:</Text>
                        <View className="flex flex-row gap-2 w-max flex-wrap">
                            {
                                run_days.map((singleClass, index) => {
                                    return (
                                        <Text className={'flex flex-row items-center justify-center px-3 py-2 rounded text-sm font-bold text-OffWhite bg-Primary w-14'} key={index}>
                                            {singleClass}
                                        </Text>
                                    )
                                })
