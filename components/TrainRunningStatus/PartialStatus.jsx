    ...
    return (
        <View className="h-[5000px] relative w-full mb-12">
            ...
            <View className={"absolute left-5 w-3 h-full rounded-full my-6"}>
                <View className={`bg-Primary rounded-full`} style={{ height: `${trainPos}%` }}></View>
                <View className={`bg-LightDark rounded-full h-full`}></View>
            </View>
            {
            ...
            <View className="absolute flex flex-row gap-1 ml-[1px]" style={{ top: `${trainPos}%` }}>
                ...
                <Animated.View className="w-3/4" key={"uniqueKey"} entering={FadeOut.duration(8000)}>
                    <Text className="w-full h-max text-Light bg-Background p-2 rounded-lg">{currentData.current_location_info[0].readable_message}</Text>
                </Animated.View>
            </View>
        </View>
    )
